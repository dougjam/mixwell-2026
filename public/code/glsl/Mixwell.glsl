...
vec3 paintTestSquare(vec2 q) { // NEW
  return vec3(1.0) * smoothstep(0.1, 0.11, q.x) * smoothstep(0.2, 0.19, q.x) *
         smoothstep(0.1, 0.11, q.y) * smoothstep(0.2, 0.19, q.y);
}

vec3 splatBlobRows(vec2 p, vec3 colBG, vec3 colBlob, float HX, float HY, float dY, float blobRadius, bool isOddRow) { // NEW
...
const vec3 paletteBluePurple[6] = vec3[6]( // NEW
    vec3(  8.,   8., 140.)/255.0,
...
//------------------------------------------------------------------------------
const vec3 paletteCoolors1[6] = vec3[6]( // NEW
...
//------------------------------------------------------------------------------
const vec3 paletteGoldenMeadow[6] = vec3[6]( // NEW
...
vec3 paintBlobsWithPalette(vec2 p, vec3 palette[6]) { // NEW
...
// Returns rdLine1DS series implementation of 1D drift. Provided for "backwards convenience." // NEW
...
// Reverse drift through Mixwell flow of radius-eps brush moving a→b. Computed using adaptive midpoint integration. // NEW
