...
float sdBox(float2 p, float2 b) {
    float2 d = fabs(p) - b;
    return length(fmax(d, (float2)(0.0f, 0.0f))) + fmin(fmax(d.x, d.y), 0.0f);
}
...
float3 quantizeColor(float3 c, float n, int bands) {
    ...
    if(bands) res *= smoothstep(-0.0001f, +0.0001f, length(c-res)-0.005f);
    return res;
}
...
float3 paintStripes(float2 q) {
    //return pow(smoothstep(0.0f, 1.0f, fract(6.0f*q.y+sin(33.0f*q.x*q.y))), 3.0f)*(float3)(1.0f);
...
//------------------------------------------------------------------------------
__constant float3 paletteCoolors1[6] =// NEW
{
...
//------------------------------------------------------------------------------
__constant float3 paletteGoldenMeadow[6] =// NEW
{
...
float3 paintBlobsWithPalette(float2 p, __constant float3 *palette)// NEW
{
...
float2 CarlsonRFn(float x, float y, float z) {
    ...
    float An = A0;
    float Q  = pow(3.0f * CARLSON_TOL, -1.0f/6.0f) * fmax(fmax(fabs(A0-x), fabs(A0-y)), fabs(A0-z));
    float xn = x;
    float yn = y;
    float zn = z;
...
float2 CarlsonRDn(float x, float y, float z) {
    ...
    float An  = A0;
    float Q   = pow(CARLSON_TOL/4.f, -1.f/6.f) * fmax(fmax(fabs(A0-x), fabs(A0-y)), fabs(A0-z));
    float xn  = x;
    float yn  = y;
    float zn  = z;
    float Sum = 0.f;
...
float3 EllipticFEn(float phi, float m) {
    ...
    float n       = fmax(RFn.y, RDn.y);// max(nRF,nRD)
...
float mxwSigmaAt(float2 p) { 
  ...
  //if(fabs(Y) < 0.0001f)    return M_PI_2 * sign(X); 
...
float mxwTimeAt(float2 p) {
  ...
  float X = p.x;
  float Y = fabs(p.y); // MirrorY: We're in the upper court, kids.
...
float mxwTimeAtSigmaEta(float sigma, float eta) {
  ...
  sigma = mxwClampSigmaPosY(sigma); // Safety first. 
  eta   = fabs(eta); // MirrorY: We're in the upper court, kids.
...
float mxwSigmaSolver(float tBar, float eta) {
  ...
  for(int iter=0; iter<20; iter++) {
    ...
    if(fabs(dSigma) < PI_2*SIGMA_NEWTON_TOL) break;    
  }
...
float2 mxwPositionSolveAtTimeEta(float t, float eta) {
  ...
  if(fabs(eta)<0.0001f) eta = 0.0001f * sign(eta);   
...
float2 mxwAdvectPointUnitPatched(float2 p, float dt, bool applyPatch) {
  ...
  if(applyPatch && fabs(etaInf_p) < etaMax) {
    // EXTRAPOLATE INWARD ON y=0 PARAMETERIZATION'S SEAM:
    bool  inWedge = (acos(fabs(normalize(p).x)) < 0.05f);
    float xCrease = sign(dt) * (0.5f + fabs(dt));
    bool inPatchRegion = (inWedge && 
                           ((dt < 0.f && (p.x >= +1.f || p.x < xCrease)) || 
                            (dt > 0.f && (p.x <= -1.f || p.x > xCrease))));
    if(inPatchRegion) {
...
float2 mxwApplyTineInsertionMap(float2 p, float r) {
  if(dot(p,p)==0.f) p = 0.000001f*(float2)(1.f); // avoids R=0 and 0/0 result.
...
float2 mxwAdvectPointUnit(float2 p, float dt, bool insertRemoveTine, bool applyPatch) {
  ...
  float cuspH     = min(0.02f, 0.005f*fabs(dt));
  if(insertRemoveTine && applyPatch) {
    if(fabs(p0.y) < cuspH && (sign(dt)*p0.x < 0.f || sign(dt)*p0.x > fabs(dt)*0.8f)) {
...
float2 rdSegmentM(float2 p, float r, float2 a, float2 b) {
    ...
    if(length(a-b)==0.f) return (float2)(0.f);
...
float2 rdTriangle(float2 p, float r, float2 p0, float2 p1, float2 p2) {
    ...
    float2 P[4] = {(float2)(p0), (float2)(p1), (float2)(p2), (float2)(p0)};
    ...
    float masking = 1.f - smoothstep(sdfMax/2.f, sdfMax, fabs(sdf));
